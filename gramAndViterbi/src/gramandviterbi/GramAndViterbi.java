...
public class GramAndViterbi {
    ...
    protected int trouveMinAlpha(int indice) {
        ...
        for (int j = 0; j < alpha[indice].length; j++) {
            if (alpha[indice][j] != null) {
                if (alpha[indice][j] < i) {
                    i = alpha[indice][j];
                    indice = j;
                }
    ...
    protected void viterbi(LinkedHashMap<String, ArrayList<String>> treillis) {
        ...
        for (int i = 1; i < treillis.size(); i++) {
            ...
            for (int j = 0; j < treillis.get(Integer.toString(i)).size(); j++) {
                ...
                double a = Double.parseDouble(estimationModele.get(getMot(treillis.get(Integer.toString(i)).get(j))));
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < beta.length; i++) {
            ...
            for (int j = 0; j < beta[i].length; j++) {
                System.out.println("case " + beta[i][j]);
            }
        }
    }
}
